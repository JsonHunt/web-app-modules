...
(function() {
  var AuthService, FastBricks, emailjs, sql, uuid;
  ...
  AuthService = (function() {
    function AuthService(dbconfigFile) {
      this.fb = new FastBricks();
      this.fb.loadConfig(dbconfigFile);
    }

    AuthService.prototype.server = emailjs.server.connect({
    ...
    AuthService.prototype.signup = function(user, callback) {
      var email, username;
      if (!user.username) {
      ...
      return this.fb.query(sql.select().from('`user`').where(username), (function(_this) {
        return function(err, result) {
          if (result.length > 0) {
            callback("This user name is not available");
            return;
          }
          return _this.fb.query(sql.select().from('`user`').where(email), function(err, result) {
            var newuser;
            if (result.length > 0) {
            ...
            return _this.sendActivationEmail(newuser, function(err) {
              if (err) {
                ...
                return this.fb.query(sql.insert('`user`', newuser), (function(_this) {
                  return function(err, result) {
    ...
    AuthService.prototype.login = function(username, password, callback) {
      ...
      return this.fb.query(expr, function(err, result) {
        var user;
        if (err) {
          callback(err);
        }
    ...
    AuthService.prototype.sendEmail = function(msg, callback) {
      return this.server.send(msg, function(err, message) {
        ...
        if (err) {
          ...
          return this.fb.query(sql.insert('email', email), function(err, result) {
            if (err) {
              return callback(err);
            } else {
              return callback();
    ...
    AuthService.prototype.sendActivationEmail = function(user, callback) {
      var msg;
      msg = {
    ...
    AuthService.prototype.activateUser = function(activationCode, callback) {
      return this.fb.query(sql.select().from('`user`').where({
        ...
        if (results.length === 0) {
          ...
          user = results[0];
          return this.fb.query(sql.update('`user`', {
            verification: 1
          }).where({
            id: user.id
          }), function(err, result) {
    ...
    AuthService.prototype.requestPasswordReset = function(email, callback) {
          ...
          if (results.length === 0) {
            ...
            return _this.sendEmail(msg, function(err) {
              if (err) {
                ...
                return _this.fb.query(sql.update('`user`', {
    ...
    AuthService.prototype.resetPassword = function(code, callback) {
      return this.fb.query(sql.select().from('`user`').where({
        password_code: code
      }), function(err, results) {
        if (err) {
    ...
    AuthService.prototype.changePassword = function(user, password, callback) {
      if (!user) {
